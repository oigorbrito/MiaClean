...
/**
 * Input validation limits to prevent resource exhaustion (DoS).
 */
...
function parseRequestBody(raw: unknown): VerifyPurchaseRequest | null {
  ...
  if (purchasesRaw.length > MAX_PURCHASES_PER_REQUEST) return null;
  ...
  for (const entry of purchasesRaw) {
    ...
    if (purchaseToken === null || purchaseToken.length > MAX_PURCHASE_TOKEN_LENGTH) continue;
