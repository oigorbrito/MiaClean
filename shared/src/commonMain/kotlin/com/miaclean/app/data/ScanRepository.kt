...
class ScanRepository(private val scanner: MediaScanner, private val md5Hasher: Md5Hasher, private val perceptualHasher: PerceptualHasher, private val imageEmbedder: ImageEmbedder, private val classifier: MediaClassifier, private val repository: MediaHashRepository) {
    fun scan(): Flow<ScanProgress> = channelFlow {
        send(ScanProgress.Running(0, 0))
        val items = try { scanner.scanAll() } catch (e: Exception) { send(ScanProgress.Failed(ScanErrorCode.PERMISSION_REVOKED)); return@channelFlow }
        if (items.isEmpty()) { send(ScanProgress.Done(0, 0)); return@channelFlow }
        items.forEachIndexed { index, item ->
            try { processItem(item) } catch (e: Exception) { if (e is ScanException) { send(ScanProgress.Failed(e.errorCode)); return@channelFlow } }
    ...
    private suspend fun processItem(item: MediaItem) {
        ...
        repository.upsert(item.copy(category = classifier.classify(item)), md5, perceptualHasher.hash(item.uri), imageEmbedder.embed(item.uri)?.joinToString(","))
    }
    private suspend fun buildGroups(): List<DuplicateGroup> {
        val exact = repository.findExactDuplicates()
        val exactGroups = exact.groupBy { it.md5 }.values.filter { it.size > 1 }.mapIndexed { i, list -> DuplicateGroup(i.toLong(), DuplicateGroup.Strategy.EXACT_MD5, list.map { it.item }, list.sumOf { it.item.sizeBytes }) }
    ...
    private fun buildPerceptualGroups(rows: List<MediaHash>, offset: Int): List<DuplicateGroup> {
        ...
        return components.values.filter { it.size > 1 }.map { b -> DuplicateGroup(nextId++, DuplicateGroup.Strategy.PERCEPTUAL_PHASH, b.map { it.item }, b.sumOf { it.item.sizeBytes }) }
    }
    private fun buildSemanticGroups(rows: List<MediaHash>, offset: Int): List<DuplicateGroup> {
        val decoded = rows.mapNotNull { row -> row.embeddingHash?.split(",")?.map { s -> s.toFloatOrNull() ?: 0f }?.toFloatArray()?.let { e -> row to e } }
        val visited = BooleanArray(decoded.size); val groups = mutableListOf<DuplicateGroup>(); var nextId = offset.toLong()
        for (i in decoded.indices) {
            if (visited[i]) continue
            val bucket = mutableListOf(decoded[i].first); visited[i] = true
            for (j in i + 1 until decoded.size) {
                if (!visited[j] && imageEmbedder.cosine(decoded[i].second, decoded[j].second) >= 0.92f) { bucket += decoded[j].first; visited[j] = true }
            }
            if (bucket.size > 1) groups += DuplicateGroup(nextId++, DuplicateGroup.Strategy.SEMANTIC_EMBED, bucket.map { it.item }, bucket.sumOf { it.item.sizeBytes })
        }
        return groups
    }
}
...
private class IntDisjointSet(size: Int) {
    private val p = IntArray(size) { it }; private val r = IntArray(size)
