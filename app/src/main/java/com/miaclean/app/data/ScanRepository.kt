...
import com.miaclean.app.data.adapter.*; import com.miaclean.app.data.classify.MediaClassifier; import com.miaclean.app.domain.ScanProgress; import kotlinx.coroutines.flow.Flow; import javax.inject.Inject; import javax.inject.Singleton
@Singleton
class ScanRepository @Inject constructor(private val scanner: AndroidMediaScanner, private val md5Hasher: AndroidMd5Hasher, private val perceptualHasher: AndroidPerceptualHasher, private val imageEmbedder: AndroidImageEmbedder, private val classifier: MediaClassifier, private val repository: RoomMediaHashRepository) {
    ...
    suspend fun loadGroups(): List<com.miaclean.app.domain.DuplicateGroup> {
        ...
        return exact.groupBy { it.md5 }.values.filter { it.size > 1 }.mapIndexed { index, list -> com.miaclean.app.domain.DuplicateGroup(index.toLong(), com.miaclean.app.domain.DuplicateGroup.Strategy.EXACT_MD5, list.map { it.item }, list.sumOf { it.item.sizeBytes }) }
    }
}
