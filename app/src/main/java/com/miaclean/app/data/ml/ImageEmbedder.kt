...
import com.miaclean.app.data.classify.ClassificationServiceUnavailableException
import com.miaclean.app.data.classify.EmptyClassificationResponseException
import com.miaclean.app.data.classify.InvalidImageException
...
class ImageEmbedderWrapper @Inject constructor(
    ...
    fun embed(uri: Uri): FloatArray? {
        val e = embedder ?: throw ClassificationServiceUnavailableException("Image embedder model not loaded")
        val bitmap = decode(uri) ?: throw InvalidImageException("Failed to decode image")
        return try {
            val mpImage = BitmapImageBuilder(bitmap).build()
            val result: ImageEmbedderResult = e.embed(mpImage)
            result.embeddingResult().embeddings().firstOrNull()?.toFloats()
                ?: throw EmptyClassificationResponseException("Empty embedding result")
        } catch (e: ClassificationException) {
            throw e
        } catch (e: Exception) {
            throw UnexpectedClassificationException("Embedding failed", e)
        } finally {
            bitmap.recycle()
        }
    }
