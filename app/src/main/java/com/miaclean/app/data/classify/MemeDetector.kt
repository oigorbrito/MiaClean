...
class MemeDetector @Inject constructor(
    ...
    suspend fun isMeme(
        ...
        val bitmap = BitmapUtils.decodeDownscaled(context, uri, RECOGNIZER_TARGET_PX)
        if (bitmap == null) {
            logger.logFailure("MemeDetector", mediaId, ErrorCategory.IMAGE_INVALID, "Decoding failed", System.currentTimeMillis() - startTime)
