...
android {
    ...
    kotlinOptions { jvmTarget = "17"; freeCompilerArgs += listOf("-opt-in=kotlin.RequiresOptIn", "-opt-in=androidx.compose.material3.ExperimentalMaterial3Api") }
    buildFeatures { compose = true; buildConfig = true }
    packaging { resources { excludes += "/META-INF/{AL2.0,LGPL2.1}" } }
    androidResources { generateLocaleConfig = true }
    lint { abortOnError = false; warningsAsErrors = false; checkReleaseBuilds = false }
}
...
dependencies {
    ...
    testImplementation(libs.junit); testImplementation(libs.json); testImplementation(libs.mockk); testImplementation(libs.coroutines.test)
    androidTestImplementation(libs.androidx.test.junit); androidTestImplementation(libs.androidx.test.espresso)
}
...
val downloadFaceDetectorModel by tasks.registering {
    val outputDir = generatedAssetsDir; outputs.dir(outputDir)
    doLast {
        ...
        try { URI(faceModelUrl).toURL().openStream().use { input -> tmp.outputStream().use { output -> input.copyTo(output) } }
            if (!tmp.renameTo(target)) { tmp.copyTo(target, overwrite = true); tmp.delete() }
        } catch (t: Throwable) { if (tmp.exists()) tmp.delete(); if (target.exists()) target.delete() }
    }
}
